// Screen.cs - Clean version with proper S/L key handling
...
namespace TurnBasedRPG
{
    public abstract class Screen
    {
        ...
        protected virtual void HandleDevNavigation(GameTime gameTime)
        {
            ...
            if (input.IsKeyJustPressed(Keys.T)) Game.ChangeState(GameState.Title);
            if (input.IsKeyJustPressed(Keys.D)) Game.ChangeState(GameState.MainHub);
            if (input.IsKeyJustPressed(Keys.M)) Game.ChangeState(GameState.Map);
            if (input.IsKeyJustPressed(Keys.B)) Game.ChangeState(GameState.Battle);
            if (input.IsKeyJustPressed(Keys.E)) Game.ChangeState(GameState.Equipment);
            if (input.IsKeyJustPressed(Keys.H)) Game.ChangeState(GameState.HeroGallery);
            if (input.IsKeyJustPressed(Keys.P)) Game.ChangeState(GameState.PlayerProfile);

            // S = Save, L = Load → both open the slot selection screen
        ...
        protected virtual void DrawDevMenu(SpriteBatch spriteBatch, SpriteFont? font)
        {
            ...
            string devText = "T=Title D=Dashboard P=Profile M=Map B=Battle E=Equipment H=Heroes S=Save L=Load F4=Exit ESC=Back";
